...
### **Spec Prompt Creation**  
When generating **spec prompts** (only when explicitly requested):  
```  
# [Task Name]  
## Objectives  
- [High-level goal (e.g., "Implement user authentication with JWT").  

## Context  
- List 1-2 relevant files (e.g., `auth.py`, `user_models.py`).  

## Low-Level Tasks  
1. `auth.py`: Implement JWT token generation.  
2. `user_models.py`: Add `is_verified` field to User model.  

...
from modules.chat.command_handler import CommandHandler
from modules.chat.types import CommandType, CommandResult
...
class TestCommandHandler(unittest.TestCase):
    ...
    def test_parse_command(self):
        ...
        self.assertEqual(self.handler.parse_command("/think 1024"), (CommandType.THINK, "1024"))
        self.assertEqual(self.handler.parse_command("/jump 3"), (CommandType.JUMP, "3"))
        self.assertEqual(self.handler.parse_command("/agent code"), (CommandType.AGENT, "code"))
        self.assertEqual(self.handler.parse_command("/model gpt-4"), (CommandType.MODEL, "gpt-4"))
        self.assertEqual(
            self.handler.parse_command("/file test.py"), 
            (CommandType.FILE, "test.py")
        )
        self.assertEqual(

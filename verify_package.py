        ...
        for file_info in wheel.filelist:
            ...
            if path.endswith('/'):
                ...
                for req_dir in required_dirs:
                    if path.endswith(req_dir) or path.endswith(req_dir.replace('/', '-')):
...
def main():
    ...
    # Check sdist file
    sdist_ok, missing_sdist_dirs, missing_sdist_files = check_sdist_files(sdist_files[0])
    if sdist_ok:
        print(f"✅ Source distribution {sdist_files[0].name} contains all required files")
    else:
        print(f"❌ Source distribution {sdist_files[0].name} is missing files/directories:")
